...
def compile_contract(root: Path = ROOT) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if required_source_fields != {"path", "lifecycle", "role", "reviewed_source_sha256"}:
        raise ContractError(f"{SCHEMA_PATH}: required_source_fields drifted")
    if required_locator_fields != {"section", "line_start", "line_end", "anchor", "fingerprint_sha256"}:
        raise ContractError(f"{SCHEMA_PATH}: required_locator_fields drifted")
    ...
    for path in card_files:
        ...
        if source_rel.is_absolute():
            raise ContractError(f"{path}: source.path must be corpus-relative")
        source_path = _resolve_repo_path(root, source_rel)
    ...
    for work in works:
        ...
        for source_record in _require_list(work.get("historical_sources"), f"{work_id}.historical_sources"):
            if not isinstance(source_record, dict):
                raise ContractError(f"{work_id}: each historical source must be a pinned object")
            source_rel_value = _require_string(source_record.get("path"), f"{work_id}.historical_source.path")
            source_rel = Path(source_rel_value)
            if source_rel.is_absolute():
                raise ContractError(f"{work_id}: historical source paths must be manifest-relative")
            source_path = _resolve_repo_path(root, source_rel, BOOK_MANIFEST_PATH.parent)
            if not source_path.is_file():
