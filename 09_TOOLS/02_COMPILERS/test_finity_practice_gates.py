...
def validate_lifecycle_evidence(
    ...
    *,
    ...
    static_root = static_root or root
    ...
    front_door = registry["front_door"]
    if not isinstance(front_door, dict) or set(front_door) != FRONT_DOOR_KEYS:
        raise ValueError("invalid front-door contract")
    _bound_file(
        static_root,
        front_door["path"],
        front_door["sha256"],
    ...
    if front_door["packet_status"] != "typed":
        raise ValueError("front door overstates packet status")
    if front_door["contact_status"] != "deferred":
        raise ValueError("front door overstates contact status")
    if front_door["authority"] != "routing_only_no_semantic_authority":
        raise ValueError("front door claims semantic authority")
...
class FinityPracticeGateTests(unittest.TestCase):
    ...
    def test_registry_schema_is_exact_routing_only_and_externally_empty(self) -> None:
        ...
        self.assertEqual(set(self.registry["front_door"]), FRONT_DOOR_KEYS)
    ...
    def test_definition_source_and_semantic_owners_are_bound_not_reassigned(self) -> None:
        ...
        for card_id in self.registry["claim_card_ids"]:
            self.assertNotIn("owner_ids", self.cards[card_id])
            self.assertEqual(self.cards[card_id]["supporting_owner_ids"], [])
    ...
    def test_all_declared_paths_are_corpus_relative_and_contained(self) -> None:
        ...
        self.corpus_path(self.registry["front_door"]["path"])
    ...
    def test_packets_match_current_draft_hashes_and_readiness_is_fail_closed(self) -> None:
        ...
        front_door = self.registry["front_door"]
        readme = self.corpus_path(front_door["path"], within=PACKET_ROOT)
        self.assertEqual(
            front_door["sha256"], hashlib.sha256(readme.read_bytes()).hexdigest()
        )
        self.assertEqual(front_door["packet_status"], "typed")
        self.assertEqual(front_door["contact_status"], "deferred")
        readme_text = readme.read_text(encoding="utf-8")
        for marker in (
            "TYPED LOCAL PACKETS ADOPTED",
            "three locally adopted",
            "every contact and execution gate still deferred or blocked",
        ):
            self.assertIn(marker, readme_text)
        for stale_marker in (
            "DETAILED PACKETS DEFERRED",
            "have not been adopted into this repair branch",
            "no claim that draft files elsewhere are part of this branch",
        ):
            self.assertNotIn(stale_marker, readme_text)
        for gate in self.gate_rows:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(ValueError, "matches original custody"):
                validate_lifecycle_evidence(
                    same_custodian, temp_root, static_root=ROOT
                )
            ...
            with self.assertRaisesRegex(ValueError, "internal project custody"):
                validate_lifecycle_evidence(
                    internal_replication, temp_root, static_root=ROOT
                )
            ...
            with self.assertRaisesRegex(ValueError, "lacks conflict disclosure"):
                validate_lifecycle_evidence(
                    missing_conflict_declaration, temp_root, static_root=ROOT
                )
    ...
    def test_claim_card_locators_dereference_definition_and_retirement(self) -> None:
        ...
        expected = {
            "FIN01-01": {
                "locator": {"section": "3B", "line_start": 145, "line_end": 164},
                "markers": (
                    "## 3B. The Finity Card",
                    "**Finity Card**",
                    "DECISION",
            ...
            "FIN01-02": {
                "locator": {"section": "3B", "line_start": 166, "line_end": 172},
                "markers": (
                    "selected practice `[S]`",
                    "remains `[C]`",
                    "retire the",
        ...
        for card_id, contract in expected.items():
            ...
            self.assertEqual(
                {key: locator[key] for key in contract["locator"]},
                contract["locator"],
            )
    ...
    def test_review_receipt_card_rows_match_v2_owners_and_scope_is_internal(self) -> None:
        receipt = RECEIPT_PATH.read_text(encoding="utf-8")
        pattern = re.compile(
            r"\|\s*`(?P<id>FIN01-\d{2})`\s*\|\s*(?P<owner>K-\d+)\s*\|"
            r"\s*`\[(?P<tier>[A-Z])\]`\s*\|"
        )
        rows = {
            match.group("id"): {
        ...
        for card_id, row in rows.items():
            self.assertEqual(
                row["owner"],
                self.cards[card_id]["semantic_owner_id"],
            )
            self.assertEqual(row["tier"], self.cards[card_id]["evidence"][0]["tier"])
            self.assertEqual(
        ...
        for marker in (
            ...
            "No reader or participant contact",
            "preregistration",
            "result",
            "replication",
