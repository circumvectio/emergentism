...
OWNER_CONTRACT_FRAGMENTS = {
    "05_COSMOLOGY/00_THE_BURRI_RULES.md": (
        ...
        "## BR-6 · Composition",
        "Removing this section must leave BR-1…BR-6 and all operational",
...
def validate_topology(
    ...
    for number in range(5):
        ...
        expected_modality = "possible" if number == 4 else "actual"
        if (
    ...
    overlay_nodes = {
        node.get("id") for node in nodes if node.get("overlay") == "quantum"
    }
    overlay_edges = {
        edge.get("id") for edge in edges if edge.get("overlay") == "quantum"
    }
    overlay_ids = overlay_nodes | overlay_edges
    expected_overlay = {
        "quantum-state",
        "quantum-record",
        "e-quantum-correspondence",
    }
    if overlay_ids and overlay_ids != expected_overlay:
        errors.append("quantum overlay must be the exact removable three-element inset")
    for edge in edges:
        if edge.get("id") in overlay_edges:
            if edge.get("from") not in overlay_nodes or edge.get("to") not in overlay_nodes:
                errors.append("quantum overlay edge cannot enter the operational graph")
        elif edge.get("from") in overlay_nodes or edge.get("to") in overlay_nodes:
            errors.append("operational edge cannot depend on a quantum overlay node")
    ...
    if set(views) != {"proof", "emblem"}:
        ...
        if views["proof"].get("showQuantumInset") is not True:
            errors.append("proof view must expose the removable quantum inset")
        if views["emblem"].get("showQuantumInset") is not False:
            errors.append("emblem view must omit the quantum inset")
    elif any(view.get("showQuantumInset") for view in views.values()):
        errors.append("a removed quantum overlay cannot remain visible in a view")
...
def _render_proof(topology: Mapping[str, Any], digest: str) -> str:
    ...
    svg.text(65, 128, "BR-2 · FIVE CANDIDATE CROSSINGS + NON-mu EXIT / RETURN", size=11, fill="#5b534c", weight=700, letter_spacing=1)
    ...
    has_quantum_overlay = any(
        node.get("overlay") == "quantum" for node in topology["nodes"]
    )
    if view.get("showQuantumInset") and has_quantum_overlay:
