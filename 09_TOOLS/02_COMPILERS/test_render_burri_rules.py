...
class TopologyContractTests(unittest.TestCase):
    ...
    def test_quantum_overlay_is_removable_and_non_load_bearing(self):
        ...
        self.assertEqual(
            overlay_ids,
            {"quantum-state", "quantum-record", "e-quantum-correspondence"},
        )
        reduced = copy.deepcopy(self.topology)
        reduced["nodes"] = [
            node for node in reduced["nodes"] if node["id"] not in overlay_ids
        ]
        reduced["edges"] = [
            edge for edge in reduced["edges"] if edge["id"] not in overlay_ids
        ]
...
class RenderingContractTests(unittest.TestCase):
    ...
    def test_proof_and_emblem_preserve_required_boundaries(self):
        ...
        for fragment in (
            "FIVE CANDIDATE CROSSINGS + NON-mu EXIT / RETURN",
            "D4 actual / enacted",
