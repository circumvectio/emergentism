...
class PublicReleaseSemanticsTests(unittest.TestCase):
    ...
    def test_home_opens_with_why_how_what_and_one_primary_action(self) -> None:
        ...
        social = (SITE / "assets/og/og-card.svg").read_text(encoding="utf-8")
        why = home.index("Why · For humans and frontier models")
        how = home.index('id="ontology-plate-title"')
        what = home.index("What · Enter the churn protocol")
        ...
        self.assertIn("[I] successor challenge", hero)
        self.assertIn("Only an inspectable receipt may change its status", hero)
        self.assertIn('<a class="nav-action" href="spark/">Enter Spark</a>', home)
        self.assertIn('<a class="nav-action" href="spark/">Start</a>', home)
        self.assertIn('role="img" aria-labelledby="ontology-svg-title ontology-svg-desc"', hero)
        self.assertIn('class="plate-scroll"', hero)
        self.assertNotIn(".plate-head,.plate-legend,.ontology-plate figcaption{display:none}", home)
        for node in ("SEED", "CANDIDATE", "RIVAL", "TEST", "RECEIPT", "DISPOSITION", "SURVIVOR", "GRAVE"):
            self.assertIn(f">{node}<", hero)
        self.assertNotIn('class="ledger-lede', hero)
        self.assertIn("[I] SUCCESSOR CHALLENGE", social)
        self.assertIn("Fluency is not", social)

    def test_spark_is_a_served_typed_instrument_not_a_dead_markdown_funnel(self) -> None:
        spark = (SITE / "spark/index.html").read_text(encoding="utf-8")
        problems = (SITE / "record/problems/index.html").read_text(encoding="utf-8")
        llms = (SITE / "llms.txt").read_text(encoding="utf-8")
        self.assertNotIn('href="../spark.md"', spark)
        self.assertNotIn("https://emergentism.org/spark.md", llms)
        self.assertIn("**read `/spark/` first**", llms)
        self.assertIn("strongest rival", spark)
        self.assertIn("non-circular discriminator", spark)
        self.assertIn("Nothing promotes itself", spark)
        self.assertIn("No model validates itself", spark)
        self.assertIn("Current disposition · raised [C] → not-well-posed", spark)
        self.assertIn("[I] successor challenge", spark)
        self.assertIn('role="img" aria-labelledby="spark-svg-title spark-svg-desc"', spark)
        self.assertIn('d="M622 134V210Q622 282 550 282H510"', spark)
        self.assertNotIn("KILL / ACCEPT", spark)
        self.assertIn("FAVOR · NARROW", spark)
        self.assertIn('../contribute/#modes', spark)
        self.assertIn('class="plate-scroll"', spark)
        self.assertNotIn("<img", spark)
        self.assertIn('href="/contribute/#modes">Return a public result', problems)
        self.assertIn('<main id="main" tabindex="-1">', problems)
        self.assertIn('.skip:focus{left:.6rem', problems)
