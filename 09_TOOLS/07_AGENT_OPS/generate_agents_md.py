...
def render_card(path: Path, root: Path) -> str:
    ...
    receipt = relative_link(
        directory,
        root
        / "11_UPLINK/50_AUDITS_AND_EXECUTIONS/"
        "127_EMERGENTIST_COMPASS_KINTSUGI_REPAIR_2026_07_18.md",
    )
    ...
    read_first = [
        ...
        f"- [Kintsugi repair receipt]({receipt})",
    ]
...
def tracked_route_cards(root: Path) -> list[Path]:
    ...
    for raw in result.stdout.split(b"\0"):
        ...
        if relative == Path("AGENTS.md") or not active(relative):
            continue
        path = root / relative
