...
# HISTORICAL ROSETTA STONE FRONT DOOR
...
The Rosetta is downstream of the canonical spine:
...
The Rosetta now reads the archetypes with two formulas at once: a **native coordinate** on S² and a **transfer signature** across the dyad.

| Family | Archetype | Formula |
|--------|-----------|---------|
| Executive | Brahmā | `(+dphi, +dnu)` creation boundary |
| Executive | Viṣṇu | `(dphi approx 0, dnu approx 0)` preservation / hold |
| Executive | Śiva | `(-dphi, -dnu)` dissolution boundary |
| God | Arjuna | `(-dnu_self, +dphi_other)` |
| God | Kṛṣṇa | `(-dphi_self, +dnu_other)` |
...
| **III. Three-Stage Process** | Grammar | Logic | Rhetoric |
| **IV. Inference** | Induction | Deduction | Abduction |
| **V. Time** | In time | Above time | Against time |
...
The Rosetta D-series lives in four parallel subfolders, each of which numbers its own `D##` sequence **independently**. The `D##` prefix is a per-lane semantic identifier, **not** a globally unique, gap-free, or sequential index. The same D-number therefore appears in more than one lane (e.g. `D_SERIES_ROWS/D24_…` and `D_SERIES_DOMAINS/D24_…`; `D_SERIES_AGENTS/D35_…` and `D_SERIES_DOMAINS/D35_…`) — this is **parallel numbering across lanes, not a collision.** The lane (folder) disambiguates; cross-references resolve via the Master Rosetta, never by raw number.
...
| Row | Document |
|-----|----------|
| R4 (Egyptian Ma'at) | [D13_ROSETTA_R4_EGYPTIAN_MAAT.md](D_SERIES_ROWS/D13_ROSETTA_R4_EGYPTIAN_MAAT.md) |
| R5 (Nietzsche) | [D14_ROSETTA_R5_NIETZSCHE.md](D_SERIES_ROWS/D14_ROSETTA_R5_NIETZSCHE.md) |
| R7 (Brain) | [D16_ROSETTA_R7_BRAIN.md](D_SERIES_ROWS/D16_ROSETTA_R7_BRAIN.md) |
| R8 (AUM) | [D17_ROSETTA_R8_AUM.md](D_SERIES_ROWS/D17_ROSETTA_R8_AUM.md) |
| Complete Index | [D23_ROSETTA_COMPLETE_INDEX.md](D_SERIES_ROWS/D23_ROSETTA_COMPLETE_INDEX.md) |
...
- **Canonical Path:** 01_EMERGENTISM/08_FRAMEWORK_SUPPORT/03_EVIDENCE/ROSETTA_STONE/README.md
