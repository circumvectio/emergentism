...
> **[K3 reconciliation — 2026-07-13, per K2-signed receipt 126.]** Ruling 8 (Rosetta) applies: the `[S]`-tiered section *"The Ektropic Gradient Across the Rosetta"* and the six-gate cross-domain check read the seven-row L1→L7 alignment as a **discovered universal / native 7-fold recurrence**. Per receipt 126 the Rosetta is a cross-domain **lens / translation-layer**, not a discovered universal, and its kill-criterion has **fired** (traditions outside the polarity-encoding family do not fit the grid; the "7" is a design choice). Read that table and Gate 2/Gate 6 as lens-mapping scoped to polarity-encoding traditions, **not** as `[S]` structural evidence of a native seven-fold recurrence; the underlying `B=sin θ` equatorial geometry is unaffected. Where local wording conflicts, the Settled Canon Registry rules. Authority: [`../00_META/00_SETTLED_CANON_REGISTRY.md`](../00_META/00_SETTLED_CANON_REGISTRY.md).
...
**seven selected** generative actions mapping the differential motions of $\Delta\Phi$ and $\Delta V$. Not five. Not eight. **Seven as the chosen `4 + 3` filing architecture, not as a mathematical forcing.** `[I]`

> 🪦 TOMBSTONE — T3 (seven-as-forced, line 36-38). The phrase "exactly seven … Not five. Not eight." and the `[S]` tier on the "7 exhaustive" claim are RETIRED. Per Amrita §II P3, receipts 126 A3-1/C2/C11, 132 §2.3: the odd-ladder and triadic-necessity arguments do not force exactly seven; seven is the **selected** `4 + 3` filing architecture. The 4 transfers × 3 phase-frames decomposition is `[I]` (interpretive, structural under stated commitments), not `[S]` (forced). The kill criterion (an eighth generator requiring a third factor or abandoning one commitment) survives scoped. Bannered 2026-07-19.
