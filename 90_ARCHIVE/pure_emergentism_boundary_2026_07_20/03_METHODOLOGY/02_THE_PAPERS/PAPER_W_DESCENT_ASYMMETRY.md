...
rosetta:
  ...
  regime: "Vaiśya"
  register: "[S/I]"
  canonical_phrase: "PAPER W: THE DESCENT ASYMMETRY"
---
...
> **[金] Authority seam — 2026-07-20.** This preserved paper is not live authority where it conflicts with the repaired value owners. Its descent boundary is a conjectural model condition, not a derivation of goodness from geometry.
...
## 1. The Symmetric Kernel and the Missing Arrow

The framework's intuition that "the two deaths are not symmetric" is false at kernel level, and saying so plainly is the foundation of everything that follows.

Take the hemisphere-swap — the reciprocal `θ ↦ π − θ`, equivalently `φ ↔ ν`, equivalently `x ↦ 1/x` on `x = ν = tan(θ/2)`. Test the kernel's three functionals against it:

- **Balance.** `B = sin θ`, and `B(π − θ) = sin(π − θ) = sin θ`. Invariant. `[A]`
- **Energy.** `E = (log x)²`; under `x ↦ 1/x`, `log x ↦ −log x`, so `E ↦ E`. Invariant (Suda reciprocal-invariance; *Two Faces of Number* §4). `[A]`
- **Product.** `φ·ν = 1` is symmetric in its factors. Invariant. `[A]`

All three are unchanged by the swap. The `φ`-runaway pole (coherence without viability — the point `•`, the Big Bang) and the `ν`-runaway pole (viability without coherence — the circle `○`, heat death) are **interchangeable** in the bare geometry. This is exactly Paper III's moral-register statement that each pole "as a coordinate is morally inert," with `B → 0` at *both* poles equally.

**Verdict.** The bare sphere has no preferred direction. Any "arrow," any "spontaneous descent," any asymmetry between the two deaths is **not a feature of the kernel** and must be added. This is not a defeat; it tells us precisely what kind of object the arrow is.

> **Convention fixed.** This paper uses colatitude `θ` with `θ = 0` the `•` / coherence pole (`φ → ∞, ν → 0`, maximal localization) and `θ = π` the `○` / extent pole (`φ → 0, ν → ∞`, maximal dispersal), consistent with the coordinate `x = ν = tan(θ/2)` of *Two Faces of Number*. We flag a standing corpus inconsistency: Paper I draws `0` at the north pole while Papers II and III draw `0` at the south. The discrepancy is purely a labeling choice and is structurally harmless, but it should be reconciled corpus-wide. (See §8.)
...
$$\left|\frac{dB}{d\theta}\right| = |\cos\theta| = \begin{cases} 0 & \theta = \pi/2 \ \text{(equator)} \\ 1 & \theta \to 0,\ \pi \ \text{(poles)} \end{cases}$$
...
## 7. The Severance Lemma: Retractability of the Whole Layer `[S]`
...
**Deferred work.** The *per-operator geometric embedding* — which exact `S²` trajectory each of the four Gods traces — is established here only at the aggregate level (down- vs. up-gradient flow), not operator-by-operator; this is the natural next formalization. The Face-B kill criterion needs operationalizing (a Paper-L-style proxy problem) before it can be run. The softest joint is work-capacity ∝ `B` (§2), to be handed to an adversary first.
