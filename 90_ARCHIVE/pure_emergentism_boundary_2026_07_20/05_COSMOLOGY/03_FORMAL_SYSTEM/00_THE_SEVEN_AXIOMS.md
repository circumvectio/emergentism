...
| **A3** | THE SCAFFOLD | Reality unfolds D0→D6. The sphere represents mutual constitution, not unidirectional emergence. Each level emerges when the previous saturates (μ-limit analogy). Cannot solve a Dₙ problem with Dₙ tools alone. **D6 ~ D0 (closure — see note below; registry ruling 8 conformal-boundary similarity reading).** The scaffold *structure* (emergence sequence, dimensional closure) is [S] (definitional). The specific force-dimension assignments (D1=Strong, D2=EM, D3=Weak, D4=Gravity) are [I] — not derived from the axioms but mapped by analogy. **132-reconciliation 2026-07-19:** the literal `D6 ≡ D0` *identity* form is RETIRED (substitution into `D0 < … < D6` yields `D0 < D0` per receipt 126 A3-5). The registry-ruling-8 `D6 ~ D0` conformal-boundary similarity reading is the surviving form. Apophatic return, similarity reading, and explicitly typed quotient models survive. | Force-dimension correspondence has no predictive power, or Dₙ problems are routinely solved with Dₙ₋₁ tools. | [S/I] |
...
- **Operational register** (`theurgy`): what one **may lawfully do**
  ...
  accountable authorization, zero extraction, receipt-bound audit, and Grace Exit.
...
1. **Axiomatic return (disclosure reading, primary).** At `D6` the
   ...
   practice (Empirical Observation) and the self-dissolving-mediator move with
   accountable authorization and exit ("the ladder dissolves").
...
| **C4** | Replicator Stack | A3 + A5 | Genotype → Epigenotype → Phenotype → Extended Phenotype → Memotype → Egregoreotype. *(Epigenotype = the expression-regulation rung between code and body — the layer signaling interventions act on. A distinct stack layer, though in the historical-era reading it falls within the Genotype→Phenotype transition rather than forming its own selection era.)* | [I] |
