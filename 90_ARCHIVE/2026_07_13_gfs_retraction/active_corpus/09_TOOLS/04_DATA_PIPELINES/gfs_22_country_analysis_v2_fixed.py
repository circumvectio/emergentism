...
SUPERSEDES: gfs_22_country_analysis.py — this v2_fixed variant corrects the phi/nu
composite variable names to match GFS Codebook v2 exactly (see construct_composites,
CORRECTED 2026-04-07). Prefer this script over the original for the codebook-v2 data file.
...
def construct_composites(df: pd.DataFrame, country: str) -> pd.DataFrame:
    ...
    """
    # ---- GFS Codebook v2 Variable Names (WAVE 2 EXTRACT) ----
    PHI_ITEMS = [
        "LIFE_PURPOSE_Y2", "WORTHWHILE_Y2", "PROMOTE_GOOD_Y2",
        "GRATEFUL_Y2", "PEOPLE_HELP_Y2", "SAT_RELATNSHP_Y2",
    ]
    NU_ITEMS = [
        "MENTAL_HEALTH_Y2", "PHYSICAL_HLTH_Y2", "INCOME_FEELINGS_Y2", "EXPENSES_Y2",
    ]
    DV_ITEMS = ["LIFE_SAT_Y2", "HAPPY_Y2"]
    COVARIATE_COLS = ["AGE_Y2", "GENDER", "EDUCATION_3_Y2"]
    # ---- END MAPPING ----

    # Reverse-code INCOME_FEELINGS (1=Comfortable, 4=Very difficult → reverse so higher=better)
    cdf = df[df["COUNTRY"] == country].copy()
    if "INCOME_FEELINGS_Y2" in cdf.columns:
        cdf["INCOME_FEELINGS_Y2"] = pd.to_numeric(cdf["INCOME_FEELINGS_Y2"], errors='coerce')
        valid = cdf["INCOME_FEELINGS_Y2"].between(1, 4)
        cdf.loc[valid, "INCOME_FEELINGS_Y2"] = 5 - cdf.loc[valid, "INCOME_FEELINGS_Y2"]
    ...
    # Domain means (force numeric coercion first)
    for col in phi_available + nu_available + dv_available:
    ...
    # Standardize within country for basic linear additive regression bounds
    cdf["phi"] = sp_stats.zscore(cdf["phi_raw"])
    cdf["nu"] = sp_stats.zscore(cdf["nu_raw"])

    # Hygiene / Motivator Topology (Absolute Absolute Limit Boundaries)
    # Map back to 0-1 absolute scales (Scale = 0-10) to preserve true '0' death states
    cdf["phi_abs"] = cdf["phi_raw"] / 10.0
    cdf["nu_abs"] = cdf["nu_raw"] / 10.0
    
    # Calculate geometric product on raw probability amplitudes
    phi_x_nu_matrix = cdf["phi_abs"] * cdf["nu_abs"]
    
    # Re-standardize solely for OLS coefficient stability comparison
    cdf["phi_x_nu"] = sp_stats.zscore(phi_x_nu_matrix)
