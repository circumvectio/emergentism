...
rosetta:
  ...
  regime: "Kṣatriya"
  register: "[B]"
  canonical_phrase: "Rosetta Stone naming doctrine"
title: "Rosetta Stone Naming Doctrine"
status: "CANON — naming governance"
evidence_tier: "[B] Canonical local naming doctrine."
---
...
**Do not renumber D_SERIES files.** The `D##` prefix is a **semantic identifier**, not a display-order cue. It ties directly into the Master Rosetta table (`00_THE_MASTER_ROSETTA.md`) and the framework-level row/domain taxonomy.
...
D13–D24: per-row Rosetta mappings. Each file is **one row of the Master Rosetta table**, applied across all domains. The `R##` inside each filename (e.g., `D24_ROSETTA_R14_PIE_COMPARATIVE_LINGUISTICS`) names the specific row being mapped.

```
D13_ROSETTA_R4_EGYPTIAN_MAAT.md
D14_ROSETTA_R5_NIETZSCHE.md
D15_ROSETTA_R6_TORUS.md
... (through D24)
...
## The "D24 collision" is a namespace difference, not a collision

`D24_ROSETTA_R14_PIE_COMPARATIVE_LINGUISTICS.md` (in `D_SERIES_ROWS/`) and `D24_PSYCHOLOGY.md` (in `D_SERIES_DOMAINS/`) share the `D24` prefix but live in **different subfolders** and mean **different things**:

- ROWS-D24 = "row 24 in the rows series" (mapping Rosetta row R14 across domains — Proto-Indo-European comparative linguistics)
- DOMAINS-D24 = "domain 24 in the domains series" (mapping all rows across the Psychology domain)

Same prefix, different semantic axis. The folder disambiguates. This is **parallel numbering**, not collision.
...
- Treat `D##` as a **semantic identifier** tied to the Master Rosetta table
- Add new D-files in the appropriate subfolder using the next D-number in that subfolder's sequence
- Document new additions in the subfolder's `00_GENERATIVE_TABLE.md` (or equivalent master) and `00_THE_MASTER_ROSETTA.md`
- Cross-reference sibling D-files via the Master Rosetta, not by raw number
...
## If this naming still feels wrong

If a future contributor or auditor feels the Rosetta-Stone naming is inconsistent and wants to "normalize" it:
...
- **Canonical Path:** 01_EMERGENTISM/08_FRAMEWORK_SUPPORT/03_EVIDENCE/ROSETTA_STONE/NAMING_DOCTRINE.md
