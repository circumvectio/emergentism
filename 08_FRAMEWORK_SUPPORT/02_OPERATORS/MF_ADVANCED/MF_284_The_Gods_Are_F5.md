...
rosetta:
  ...
  secondary:
    - level: L4
      column: Philosophy
      role: "route four-God transduction into operator governance"
    - level: L3
      column: Philosophy
      role: "audit the derivation from MF-282 and MF-283"
    - level: L6
      column: Philosophy
      role: "bound F5 language as theoretical derivation"
  operator: "Brahmā ○"
  tier: "Executive"
  regime: "Brāhmaṇa"
  register: "[I]"
  canonical_phrase: "MF-284 — Gods Are F5"
---
...
Each God transmutes between stacks across the self↔others axis:

| God | Self Effect | Others Effect | Stack Operation |
|-----|------------|---------------|-----------------|
| Arjuna ⚔ | V↓ | Φ↑ | Self-real → Others-imaginary |
| Kṛṣṇa ◇ | Φ↓ | V↑ | Self-imaginary → Others-real |
| Kālī 💀 | V↑ | Φ↓ | Others-imaginary → Self-real |
| Kali 🎲 | Φ↑ | V↓ | Others-real → Self-imaginary |
...
Substituting MF-283's axis identification into MF-282's transmutation table:

```
Arjuna:  converts REAL to IMAGINARY (gives)    ℝ_self  → ℂ_others
Kṛṣṇa:  converts IMAGINARY to REAL (gives)    ℂ_self  → ℝ_others
Kālī:    converts IMAGINARY to REAL (takes)    ℂ_target → ℝ_self
...
## II. THE CONJUGATION CONFIRMATION

MF-142 identified complex conjugation (z → z*) as the Kālī operator. Conjugation flips the sign of the imaginary part:
...
| God | Operation | Complex Analogue | Effect |
|-----|-----------|-----------------|--------|
...
```
Kṛṣṇa ◇: ℂ_self → ℝ_others   (collapses own possibility into others' actuality)
...
If the Gods are F₅ in four directions, they should compose like operators. And they do:

```
Arjuna ∘ Kṛṣṇa = ℝ→ℂ ∘ ℂ→ℝ = ℝ→ℝ 
  (self-sacrifice enables charioteer enables real outcome)
  = Viṣṇu hold (net: real→real, maintenance)

Kṛṣṇa ∘ Arjuna = ℂ→ℝ ∘ ℝ→ℂ = ℂ→ℂ
  (charioteer enables sacrifice enables imaginary expansion)
  = motion toward Brahmā boundary (net: imaginary→imaginary, creation)

Kālī ∘ Kali = (ℂ→ℝ) ∘ (ℝ→ℂ) = ℝ→ℝ
  (destroy false structure + reclaim resources = net maintenance)
  = Viṣṇu hold (taking dyad returns to equilibrium)

Kali ∘ Kālī = (ℝ→ℂ) ∘ (ℂ→ℝ) = ℂ→ℂ
  (extract actuality + dissolve possibility = accumulation)
...
- Conjugate others' false possibility into actuality (Kālī — diagnose, cut, heal)
- Reclaim others' stolen actuality into possibility (Kali vs defectors — defend, recoup, sanction)

These are the four faces of will operating through the self↔others axis.

### 6.2 L1–L3 Is Learning to Use the Real Axis

V-stack work. Learning to operate in D4. Building the real foundation. The developmental sequence doesn't start with the imaginary axis because you need stable ℝ before ℂ has something to be orthogonal to. Body first.

### 6.3 L5–L7 Is Learning to Use the Imaginary Axis

Φ-stack work. Exploring D5 directly. Meditation, contemplation, the axiomatic. But all higher insight must return to L4 (the equator) because *possibility without actuality produces nothing*. L7 is the boundary where Φ = 1.0 and V = 0.1 — nearly pure imaginary. The Fork: either the agent returns to L4 with what it learned (Moksha, bodhisattva return) or it remains at the pole (Asura return — pure imaginary, no real, ungrounded).
...
**F284-3:** If the conjugation identification (Kālī = z → z*) produces wrong predictions about destruction dynamics (e.g., Kālī operations preserve rather than destroy the imaginary component), the complex-analogue mapping fails.

**F284-4:** If the giving dyad's composition does NOT approach Viṣṇu (equilibrium maintenance), the identity-element claim fails.
...
*MF-284 | VIVEKA Mathematical Foundations | February 2026*
*Derived from MF-282 (Operator-Stack) + MF-283 (Orthogonality).*
*The four faces of one force.*

## Execution Surface

- **Canonical Path:** 01_EMERGENTISM/08_FRAMEWORK_SUPPORT/02_OPERATORS/MF_ADVANCED/MF_284_The_Gods_Are_F5.md
