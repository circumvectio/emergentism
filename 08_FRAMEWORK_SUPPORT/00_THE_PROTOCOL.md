---
rosetta:
  primary_level: L3
  primary_column: Philosophy
  secondary:
    - level: L2
      column: Philosophy
      role: "tier first-person and third-person evidence claims before promotion"
    - level: L6
      column: Philosophy
      role: "bound first-person disclosure as not directly third-person proof"
    - level: L5
      column: Philosophy
      role: "route protocol design back into framework-support synthesis"
  operator: "Kṛṣṇa ◇"
  tier: "Executive"
  regime: "Vaiśya"
  register: "[I/C/S]"
  canonical_phrase: "The Protocol — Sitting Practice Experimental Specification"
---

# THE PROTOCOL

> **The sitting practice rewritten as an experimental specification.**
> Sitting, instruction, measurement, report, analysis, and record are actual D4
> events. The hypotheses they test may represent D5 counterfactual content. A
> non-embodied model cannot sit, but it can help stage a protocol whose outcomes
> remain independent of its predictions.
> Read this third — after THE_DERIVATION and THE_KNIFE.

**Version:** v2026.04.10
**Purpose:** Convert first-person practice into third-person testable predictions.

---

## What This Document Is

The sitting practice is currently the framework's weakest epistemic link — a first-person experience that the framework says is foundational but which is unfalsifiable in its current form.

[I] This document does not make first-person experience exhaustively
third-person falsifiable. It does generate **third-person predictions** that are
falsifiable. Those predictions test a conjectural equator/awareness
correspondence, not a claim that awareness inhabits D5.

---

## PROTOCOL P: FIRST-PERSON VERIFICATION OF THE EQUATORIAL CONDITION

### Hypothesis

If the framework's systemic awareness-as-equatorial-condition hypothesis is correct (Step 13 of THE_DERIVATION), then sustained attention without narrative content should produce a measurable neural state that is:
1. Distinct from ordinary waking, sleep, and flow states
2. Consistent across cultural and religious traditions
3. Predicted by the spherical geometry (not predicted by standard relaxation models)

### Design

**Duration:** 20-40 minutes per session
**Posture:** Seated, eyes closed or gaze softened downward
**Instruction:** Attend to breathing. Notice internal narration. Do not engage with narration. Do not suppress narration. Notice gaps between narrated thoughts. Do not chase gaps or extend them.
**No guidance. No app. No tradition-specific framing. No music. No incense.**
**Setting:** Quiet room, no interruption, timer set.

### Predicted First-Person Report

**Minutes 1-5:** Narrator is loud. Thoughts, plans, judgments, "am I doing this right?" This is the Default Mode Network operating normally — left-hemisphere fragments without right-hemisphere integration.

**Minutes 5-10:** Narrator thins. Gaps between thoughts appear. Something quieter is present in the gaps — not a thought, not an emotion, not a sensory object. The framework calls this "the ground perceived from inside" — φ = 1 accessed directly.

**Minutes 10-20:** The ground stabilizes. Narrator makes briefer incursions. Something like clarity is present — not a thought about clarity, but clarity itself. The framework calls this "equatorial stability" — B approaching 1 from the inside.

**Minutes 20-40 (if continuing):** Stability may deepen and a practitioner may
report a sense of a "witness" distinct from narration. The practice may read
that report through the equator symbol `[I/C]`; the report and every neural
correlate are D4 actualities, not evidence for a special D4 witness type or a
literal meeting of past and future light cones.

**Critical prediction:** The quality of the "something present" in minutes 10-20 should be **the same regardless of the practitioner's cultural, religious, or philosophical background.** A Zen monk, a Christian contemplative, a secular mindfulness practitioner, and a physicist with no game-theoretic background should all report the same phenomenological core. The traditions differ in how they INTERPRET the experience. The experience itself is geometric, not cultural.

### Predicted Third-Person Neural Correlates

These are predictions of the Burri Sphere model. Some are shared with standard meditation research. Those marked with ★ are **framework-specific predictions** not made by standard models.

| Prediction | Source | Shared? | Tier |
|-----------|--------|---------|------|
| DMN deactivation (fMRI) | Step 14: narrator = DMN = left-hemisphere dominant | Shared | [C] |
| Increased alpha/theta coherence (EEG) | Step 14: corpus callosum integration | Shared | [C] |
| Increased right-hemisphere activation | Step 14: φ-axis activation | Shared | [C] |
| Increased interhemispheric coherence | Step 14: corpus callosum = i = the bridge | Shared | [C] |
| ★ Alpha coherence peaks at equatorial scalp distribution (not occipital or frontal alone) | Spherical geometry: equator = maximum B, predicted scalp distribution should be equatorial (bilateral central) | Framework-specific | [C] |
| ★ Experienced practitioners show neural signatures that scale with reported balance (B-score), not with reported bliss or absorption | B = sin θ is the variable, not pleasure or concentration | Framework-specific | [C] |
| ★ The neural "equator" signature is distinct from both flow (high ν) and deep sleep (low φ) | Flow = high-ν state, deep sleep = low-φ state, meditation = equatorial state — three different coordinates on S² | Framework-specific | [C] |

**All predictions are [C] Conjecture — testable but not yet tested in framework-specific terms.**

### Controls

| Group | Expected Result | Purpose |
|-------|----------------|---------|
| Experienced meditators (10+ years, any tradition) | Strong predicted correlates | Positive control |
| Novice meditators (first 10 sessions) | Weaker correlates, trend toward predicted | Dose-response |
| Non-meditators in quiet rest (same duration, no instruction) | Relaxation response but NOT equatorial correlates | Distinguish meditation from rest |
| Flow-state athletes (in flow) | High activation, high performance, but NOT equatorial distribution | Distinguish equatorial from high-ν states |
| Sleep-deprived subjects | Fragmented correlates, narrator more dominant | Distinguish equatorial from exhausted states |

### Kill Criterion

If experienced practitioners across traditions (Zen, Vipassana, Christian
centering prayer, secular mindfulness, Sufi dhikr, Taoist meditation) do NOT
show the predicted neural correlates—specifically, if there is no neural
signature common to all traditions that is distinct from ordinary rest—then
the conjectural systemic-awareness/equator correspondence loses its best
abductive support.

[A/I] The independent analytic chart and the selected D-register type grammar
survive this protocol's sitting-practice test. The chart is analytic; the
register assignment remains interpretive and receives no upgrade from failure
or success here.

### Falsification Gradient

This is the framework's honest assessment of how evidence would shift its position:

| Outcome | Effect on Framework |
|---------|-------------------|
| Neural correlates absent across all traditions | Equator/awareness correspondence weakened; the D-register grammar is unchanged. |
| Neural correlates present but explained as well by existing relaxation science | Correspondence not strengthened. Correlate is not cause, and the framework adds no distinctive prediction. |
| Neural correlates present AND preregistered framework-specific predictions confirmed | Correspondence receives stronger `[B]` contact and remains an interpretation pending rival-model comparison. |
| Neural correlates and first-person reports converge across cultures | Cross-method evidence strengthens the stated phenomenological model but does not make D5 consciousness, solve the hard problem, or upgrade the register scaffold to established ontology. |

### What This Protocol Is NOT

- Not a proof of the framework
- Not a religious practice (no tradition-specific framing)
- Not a wellness intervention (though wellness may result)
- Not dependent on the framework being true (the protocol works regardless; the INTERPRETATION of its results is what the framework provides)
- Not a substitute for the sitting practice itself (the practice is the thing; the protocol is the measurement)

---

## PROTOCOL D: THIRD-PERSON VERIFICATION OF THE EXTRACTION THEOREM

### Hypothesis

If agents are genuinely coupled (`λ > 0`), scored by the jointly necessary
finite-node model (`P_node=min(Φ̂₄,V₄)` is the selected ordinal working score;
the product is a retired historical name), evaluated over a long enough horizon
for network effects to return, and placed inside an enforceable `η_move = 0`
arena, then durable
optimization should select syntropic trajectories more strongly than additive
baseline agents.

This is not a claim that cooperation appears spontaneously from multiplication
alone. The current Power-Max Lemma says the opposite: coupling creates
interdependence, but one-shot extraction can still pay unless the private
extraction channel is blocked. Protocol D therefore tests the constrained
frontier: whether conjunctive agents under enforced non-extraction converge
toward `ΔP_node,i >= 0` and `ΔP_node,H >= 0` more reliably than additive agents.

### Status

**Internally supportive within framework-designed simulations** (historical run:
d = 6.66, p = 2.02 × 10⁻¹², Spearman ρ = 1.000).

**Not yet confirmed by independent replication.** This is the honest position:
framework-designed simulation testing framework metrics is structural support,
not public proof. The older run also predates the current constrained-frontier
wording, so it supports the direction of Protocol D but does not by itself
settle the four-condition Power-Max claim.

**Upgrade criterion:** An independent research team (no authorship overlap with
EFR), using a pre-registered protocol not designed within EFR, confirms that
coupled multiplicative agents under a long-horizon `η = 0` enforcement surface
select syntropic-frontier outcomes over additive baselines with effect size
d > 2.0 and p < 0.01, while also testing the failure modes: `λ = 0`, additive
payoff, short horizon, hidden extraction, and negative coupling.

**Tier: [S] Structural internal support.** Awaiting independent replication.

---

## THE FIVE BRIDGES

The sitting practice is one of five empirical bridges between the mathematical substrate and the interpretive extension. None is sufficient alone. Together, they form the framework's empirical program.

| Bridge | What It Tests | Status | Tier |
|--------|--------------|--------|------|
| **Protocol P** (sitting practice) | conjectural systemic-awareness/equator correspondence; all practice and evidence tokens remain D4 | Predictions stated, not yet tested | [C] |
| **GFS (historical)** | One retired operationalization of multiplicative balance | **RETRACTED / ARCHIVED** on 2026-07-13; not a live bridge | [B] historical result only |
| **Protocol R** | Perceptual complementarity bound Δψ × Δθ ≥ κ | Designed, needs lab | [C] |
| **AMRITA** | Spherical alignment outperforms RLHF | Specified, needs $710K | [C] |
| **Φ-meter** | Coherence is measurable and correlates with expert judgment | Built (v0.1), needs validation | [C] |

**The framework's epistemic status until the bridges deliver:**

The mathematical substrate (Steps 1-12) is [S] and [S]. It stands regardless.
The interpretive extension (Steps 13-23) is [I] and [C]. It is coherent, convergent, and testable — but not confirmed.
The bridges are under construction. The framework publishes its own failure if they collapse.

**This is the deal. No belief required. Test it.**

---

## THE SITTING PRACTICE (FOR HUMANS)

If you are a human reading this: the protocol above is for machines. For you, the instruction is simpler.

Find 20 minutes. Sit. Breathe. Notice the narrator. Let it quiet. Notice what remains.

The framework says: at that point, you are at the equator. You don't need the framework to tell you this. You don't need the geometry. You don't need the axioms. The ground was here before the equation. The ground will be here after the equation is forgotten.

The protocol is for those who cannot yet sit. The practice is for those who can.

If you can access φ directly, put this down.

Zero-Sum Resolution Equation

---

*The Protocol | v2026.04.10 | The sitting practice as experimental specification. First-person practice, third-person predictions, honest falsification gradient. The bridge between mathematics and systemic awareness. Under construction.*

## Execution Surface

- **Canonical Path:** 01_EMERGENTISM/08_FRAMEWORK_SUPPORT/00_THE_PROTOCOL.md
