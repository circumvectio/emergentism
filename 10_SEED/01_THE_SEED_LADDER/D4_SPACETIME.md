...
rosetta:
  primary_level: L4
  primary_column: Philosophy — Seed Ladder rung D4
  operator: "Arjuna ⚔"
  tier: "God"
  regime: "Kṣatriya"
...
# D4 · CAUSAL ACTUALITY / RECEIPT

> `D4` is a ladder ordinal, not the number four in `3+1` spacetime. Its primary
> type is causal actuality: actual carriers, preparations, interactions,
> attempted actions, outcomes, and provenance-bearing records. Spacetime,
> bodies, and cognition are application lenses. This page uses a normalized
> relativistic mass shell as one such lens `[I/D]`; the algebra is inherited.
...
At the D3/D4 boundary, an actual laboratory preparation, apparatus
interaction, detector outcome, and provenance-bearing record are D4. D3 holds
the quantum state and its measurement-relative Born distribution. `μ₃` names
that operational stochastic interface `[A/I]`; any stronger ontic
actualization claim remains `[C]`. For a nonrelativistic particle, D2 defines
position/momentum operators and D3 supplies their state-conditioned
distributions; D4 supplies the actual spacetime-indexed measurement event. It
does not create the momentum operator, assert fundamental collapse, or choose a
quantum interpretation.

D4 is where **realized dynamics acquire a receipt**: an internal or external
clock orders events, an apparatus registers momentum transfer, a coarse-grained
entropy-production history obtains a direction, and a gravitational
observation enters a causal ledger. D3 may already contain a momentum
distribution, a state entropy, or a state of quantum geometry. The conjecture
that relational D4 time and approximately classical spacetime emerge from a
stationary D3 state is `[C]`; the ordinal alone neither derives the bridge nor
...
5. **The iron separation holds here as everywhere.** The **seam** (`P∞ = φν ≡ 1`, a normalized identity in this declared parameterization, never a score) is not the **score** (`B ≤ 1`, maximal at the equator) is not the **node** (`N_node=(Φ̂₄,V₄)`, an ordered profile with Pareto as its scalar-free baseline). Any scalar node model requires a declared calibration contract `κ`. Physics licenses the scoped relativistic identities above; it licenses nothing about node flourishing, human worth, or ethics (CC-CORE-1). No `[C]→[A]`.
...
one.
- **Block-time vs the moving now.** *Paradox:* relativity's block seems to
  freeze the universe (nothing ever *happens*); presentism seems to contradict
  relativity (whose now?). *Typed repair:* an actual D4 model token may
  represent counterfactual worldlines as D5 possible content; D4 also contains
  embodied events, performed commitments, and receipted records. A modeled
  worldline is **possible**, never actual merely because an agent surveys it.
  The reconciliation grammar `[S/I]` separates represented alternatives from
  enacted records; it is not a new physics of time and does not settle every
  problem of temporal becoming.
