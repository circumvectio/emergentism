...
1. **Seam ≠ score ≠ node.** `P∞ = φ·ν ≡ 1` is analytic evidence inside the declared chart and empty of external-world content; it is never evidence for a node model, an empirical conservation law, or an ethic. `B = sinθ ≤ 1` is a variable score, maximal at the equator. `P_node:=min(Φ̂₄,V₄)` is the selected finite-node AND-class score over ordinal factors—selected `[I]`, structural at its zero boundary `[S]`, and conjectural as a universal fit `[C]`. The historical product `Φ̂₄V₄` / public `ΦV` is retired as a ranking. Three objects, three tiers, no silent bridge.
