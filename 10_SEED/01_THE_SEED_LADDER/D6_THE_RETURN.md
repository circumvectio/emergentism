...
rosetta:
  primary_level: L7
  primary_column: Philosophy — the seed ladder, top marker
  operator: "Viṣṇu ⊙"
  tier: "Executive"
  regime: "Ṛṣi"
...
> registers: distinction, configuration, quantum probability-state, causal
> actuality, and counterfactual possibility. Matter→bond→life→mind→choice is a
> removable content projection `[I/C]`. D0 and D6 are boundary markers.
...
*What is actually asserted here, at tier, with provenance marked.*

- **The scaffold is selected, not discovered** `[I]`. Five positive rungs and five apertures
  are a chosen taxonomy. D6 is the scaffold's chosen exit marker — our naming, our act.
- **`D6≈roleD0`** `[I]`: resemblance of *function* (limit-marking), with no
...
- **Inherited, not ours:** the incompleteness family (Gödel, Tarski) is *used, not
  ...
  **not** lift to "every D_n problem needs D_{n+1} tools" (that claim is dead — Presburger
  arithmetic and real-closed fields are complete and decidable). Wittgenstein 6.1 is
...
- **The ladder-worship failure.** Mistaking the exit marker for a sixth prize; climbing D6
  ...
  at `[I]`. A summit that were literally the ground would make the climb a circle and the
  climber a fool. The similarity is the honest residue: you return *changed*, carrying the
  record — the spiral, not the circle.
...
was always the design. A worldview whose door is painted on is a cage; ours marks the
exit on the inside.
...
freedom). The fourth posture *is* D6 practiced. The grace-exit refusal is the fourth
posture written into constitution: no entrapment, assets redeemable, the leaving blessed.

So the epistemology of exit, in three sentences a stranger can keep:
