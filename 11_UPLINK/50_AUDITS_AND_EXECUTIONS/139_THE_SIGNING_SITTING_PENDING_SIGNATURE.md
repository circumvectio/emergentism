...
*Name each box adopted or declined. Every unnamed box remains `[D]`. Mutually
exclusive choices must be named; no default may be inferred. This receipt can
adopt exact content bytes, but it cannot silently authorize physical migration.*

> **Correction after L6/L7 review.** The earlier sitting mixed doctrine adoption,
> fork defaults, registry edits, file moves, public synchronization, and deletion.
> That form was not safe to sign. Exact candidate bytes are now listed in the
> signing manifest above. The file/folder manifests remain planning drafts with
> owner-review debt; no execution wave is included in Boxes 1–9.
