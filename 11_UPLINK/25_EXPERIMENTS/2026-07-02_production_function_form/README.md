...
- `run_prodfn.py` — fits & 10-fold-CV-compares: additive · Cobb-Douglas(free) · product-unit(Φ×V) · Leontief-min · CES(ρ grid). Pre-registered kill-criteria PF1–PF3.
- `RUN_OUTPUT.txt` / `VERDICT.md` — result: **all four kill-criteria FAIL**; the symmetric balanced product is decisively rejected (asymmetric elasticities a=0.61≠b=0.44; CES ρ≈−0.9 near-linear; unit product badly misspecified). `[B]` result, `[I]` interpretation. No signature or adoption gate changes the result.
