...
An agent may represent alternative futures in a fallible D5 model, rank them,
and combine a selected alternative with D4 means to commit an action. The
...
`D6 ~ D0` means self-similar or apophatically reminiscent, never numerical or
logical identity. There is no `mu_5`, no `mu_6`, and no inference `D0 < D0`.
