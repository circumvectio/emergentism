...
rosetta:
  ...
  secondary:
    - level: L3
      column: Philosophy
      role: "keep Möbius-map composition separate from sovereign Titan frames"
    - level: L6
      column: Ontology
  ...
  regime: "Brāhmaṇa"
  register: "[A/S/I]"
  canonical_phrase: "Pole-Axis Möbius Composition — maps compose; Titans do not"
title: "Pole-Axis Möbius Composition Correspondence"
status: "SUBORDINATE KINTSUGI NOTE — former Titan-operation reading superseded 2026-07-21."
evidence_tier: "[A] Möbius-map algebra in the declared subgroup; [I] the balance/operator correspondence; no Titan operation."
depends_on:
...
# Pole-Axis Möbius Composition Correspondence

> **[金] Sovereignty seam.** The earlier version called composition of named
> Möbius maps a “Titan Composition Law” and visually identified its result with
> the Titan emblem. That was a type error. In PSL(2,ℂ), **maps** compose and
> their complex multipliers multiply. Sovereign `TitanFrame` terms do neither.
> The conditional Möbius result survives `[A]`; its operator naming is `[I]`.

---

## 1. Separate frame terms from maps

The Settled Canon Registry rules:

```text
TitanFrame ↛ Number
add_T, sub_T, mul_T, div_T : undefined
```

This note concerns a different type:

| Register | Actual operation | Status |
|---|---|---|
| TitanFrame | none; three opaque framing terms | settled `[S/I]` |
| Numeric reciprocal chart | multiplication of positive coordinates `φν=1` | `[A]` by definition |
| Möbius subgroup (this note) | composition `M_σ∘M_κ=M_{σκ}` | `[A]` within the declared model |

Map composition may be compared aesthetically with the frame emblem, but it
does not make the emblem a function, operation, or theorem.

## 2. Setup `[A]`

Fix the projective pair `{0,∞}` on `Ĉ`. These are mathematical points selected
as neighbors of the ground/horizon frame labels; they are not `0_T` or `∞_T`.
The Möbius maps fixing both points are exactly the diagonal maps
...
1. **Conditional map result.** The composite is elliptic precisely when the expansion and contraction magnitudes close reciprocally. This is a theorem about `M_σ∘M_κ`, never an evaluation of a Titan expression.
...
## 4. The unbalanced map cases `[A/I]`
...
The Coupling Law may use these two directions as a correspondence `[I]`. The
algebra itself says only that an unbalanced **pair of maps** retains expansion
or contraction. It establishes no Titan dynamics or ethic.
...
Scope fence: the result lives in the subgroup fixing the declared projective
pair `{0,∞}`. Compositions of hyperbolic maps with different axes are outside
this note's claim. Nothing here establishes a unique “world-axis.”
...
3. If any reader identifies map composition with a Titan operation or uses the
   result to derive the emblem, the correspondence fails its type contract.
...
This research note grants no execution authority; repository routing lives only
in the applicable `AGENTS.md` and `CLAUDE.md` files. Always state the map-domain
condition `|σκ|=1`. Never insert a binary operator between the sovereign Titan
seats or report a map-composition theorem as a Titan law.
