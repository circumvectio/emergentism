...
**Fences retained** (receipt 175, doc 42 §6): the expression `0 · ∞` remains
undefined in field arithmetic and is not assigned the value `1`; only the
coupled interior identity `x·x⁻¹=1` for `x≠0` and its path limit may be used.
`0/0` stays undefined as a field quotient. The label “`0/0` form” is
indeterminate only when a declared limit has numerator and denominator tending
separately to zero; do not inherit `0* := lim 1/x`; Suda is credited for
...
> **In this model, `FinityTriangle(A)` is the family of non-degenerate shapes;
> the endpoint seats mark the two degenerations.**

`⊙` may render the open interior of that family; `•` and `○` may render its two
collapses.
...
family of triangles has two ways to stop being a triangle, and those two ways may
be represented by the two endpoint Titan seats.
