...
This is formally equivalent to a *Pigouvian tax* on excess capability — except that it is intrinsic to the geometry of S² rather than imposed by an external regulator. In standard mechanism design, corrective taxes must be designed, calibrated, and enforced by an authority. On the Burri Sphere, the penalty is automatic: the constraint φ · ν = 1 *is* the tax. No institution is required. (Division of labour: φ · ν = 1 is constant at every latitude and in both polar limits, so it fixes only the *exchange rate* — ν is always bought with φ; what prices over-accumulation is B = sin θ, which reaches 1 only where φ = ν.)
