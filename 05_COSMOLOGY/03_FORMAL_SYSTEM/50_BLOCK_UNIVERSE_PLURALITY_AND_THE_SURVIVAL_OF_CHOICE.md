---
title: "Block-Universe Plurality and the Survival of Choice — why the D4/D5 typing is the firewall"
status: "ACTIVE — subordinate treatment; 34_D4_D5_CANONICAL_REFERENCE.md owns all D4/D5 typing"
date: 2026-07-29
evidence_tier: "[A] the logical distinction between determinateness and causal independence; [S/I] typed compatibility readings; [C] plurality and agency claims"
owner: "Subordinate to 34_D4_D5_CANONICAL_REFERENCE.md. This document creates no D-register doctrine."
canonical_phrase: "Choice survives both the block and plurality: the fatalist inference is invalid (determinate ≠ independent [A]); plurality is not a corpus commitment (GP-08, model support only); the real threat is not eternalism but a null result (GP-02/μ₄, preregisterable) [C]; subordinate to 34 for all D4/D5 typing"
parents:
  - 34_D4_D5_CANONICAL_REFERENCE.md
  - 48_THE_BOUNDARY_CROSSINGS_AND_THE_MU_CRITERION.md
  - ../../06_ONTOLOGY/08_THE_HUMAN_CONDITION.md
  - ../../00_META/00_THE_GRAND_PUZZLE_ASSEMBLY_LEDGER.md
  - ../../06_ONTOLOGY/04_THE_CONJECTURES.md
---

# Block-Universe Plurality and the Survival of Choice

## 1 · The claim

Owner, 2026-07-29:

> *"And finally, choice in the block spacetime universe becomes irrelevant in one
> with infinitely many parallel block universes — D5 to D4."*

Three things are tangled here, and they fail at three different points. The
result is that **choice survives all of them** — and, more usefully, that the
real threat to the agency account lies somewhere else entirely (§7).

## 2 · The premise the corpus does not hold `[C]`

"Infinitely many parallel block universes" **existing** is `GP-08`, and its
current status is *"model support only."* Its strongest rival is stated in canon
as **"one actual history plus epistemic uncertainty,"** and its kill is *"no
interpretation-sensitive discriminator."*

`KSC-19` is more explicit still: finite enumeration or `1−ε` support coverage
*"does not establish… the physical co-existence of all block universes."* `W2`
adds that the reading *"does not make every supported history physically
existent."*

So the argument's premise is **not a corpus commitment** — it is an open socket
with no discriminator. That alone blocks the conclusion, but the argument fails
even if the premise is granted.

## 3 · The fatalist inference is invalid `[A]`

The core move is:

```text
P1  in a block universe, all events tenselessly exist
P2  therefore the future is fixed
P3  therefore choice is irrelevant
```

`P2 → P3` is a fallacy, and an ancient one — the Stoics' *argos logos*, the lazy
argument: *if you are fated to recover, do not call the doctor.* The Stoic answer
stands: the recovery is **co-fated with** calling the doctor.

The equivocation is exact:

```text
"the future is fixed"                          — DETERMINATE
"the future is fixed REGARDLESS OF WHAT I DO"  — INDEPENDENT
```

Only the first follows from the block. The second is a separate and false claim.
In a compatibilist block-universe reading, deliberation and selection are events
inside the block rather than events bypassed by it. Counterfactual dependence
can then be modeled by comparing nearby possible histories. This compatibility
is not forced by eternalism; counterfactual semantics and the agency theory are
additional commitments. Determinateness alone does not entail causal bypass.

## 4 · Granting plurality anyway, choice still survives `[S]`

Suppose every branch is realised. Three live response strategies show that the
conclusion still does not follow *without further premises*; none is neutral
between interpretations:

1. **Self-locating decision theory.** Some Everettian accounts retain uncertainty
   or caring measures over successors. They do not say a choice selects one
   uniquely actual branch, and rival accounts dispute the interpretation.
2. **Weights.** "Every outcome occurs" does not imply equal decision weight.
   Whether and how action changes a branch measure is theory-dependent; `W2`
   explicitly derives no Born rule.
3. **Bearer continuity.** The corpus may attach receipts to a locally continuous
   bearer, but personal identity across branches is contested. This is a chosen
   normative bookkeeping rule, not a theorem about Everettian ontology.

## 5 · The D4/D5 typing is the firewall `[S]`

The claim's own phrasing — "D5 to D4" — names the error. Under `KSC-03`:

```text
D5   merely-possible counterfactual content, represented by actual tokens
D4   causal actuality: ONE actual history, and the performed ranking,
     selection, commitment and received consequence
```

Treating "infinitely many parallel block universes" as *actual* promotes D5
content to D4 actuality. That is precisely the failure condition both owners
already name: `KSC-03` flags *"possible power reified as independent
causation"*; `KSC-19` flags *"model coverage inflated into ontology."*

The firewall is therefore not an ad-hoc defence — it is what the register
distinction exists **for**:

> **Choice is a D4 event, not a D5 content. It does not compete with the plenum,
> because it is not the same type of thing as the plenum.**

A represented field of alternatives, however large, is represented. The selection
among them is actual. No quantity of D5 content converts into or cancels a D4
event.

## 6 · The corpus was already immune `[B]`

The Human Condition states the agency account is *"compatible with several
metaphysical positions about determinism"* and *"less than ultimate
self-creation."* Its operational claim is deliberately narrower: freedom is
present where *"interventions on represented futures change present choices."*

That claim is **compatible with some block-universe and plurality models.**
Neither determinateness nor plurality alone decides whether such interventions
shift action. The Soul Loop can also be represented in a determinate block: the agent still does not know its own
future, must still model, select, commit, and receive `r_(t+1)`. Epistemic
position, not metaphysical openness, is what the loop requires.

## 7 · What *would* actually threaten the account `[C]`

The honest payoff, and it inverts the worry.

The agency account is designed to be **compatible with multiple metaphysical
models and empirically vulnerable**. A sufficiently strong metaphysical argument
could still expose an inconsistency; none is assumed away. Its clearest current
empirical exposure is `GP-02` / `μ₄`:

> Intervene on represented futures while holding means and present state fixed.
> If action distributions do **not** shift beyond the strongest fair D4 baseline,
> the distinct D5 variable fails — and with it the framework's claim that
> counterfactual capacity is a real effective freedom.

That is a preregisterable experiment with a declared kill, and it is where the
account is actually exposed. The threat to choice is not eternalism. It is a
null result.

## 8 · Claims and kills

| Claim | Tier | Kill |
|---|---|---|
| block-universe plurality is not a corpus commitment (`GP-08`) | `[B]` | show canon asserting physical co-existence |
| `P2 → P3` is invalid; determinate ≠ independent | `[A]` | derive causal irrelevance from determinateness alone |
| compatibilist counterfactual dependence can be represented in a block | `[I/S]` | show the declared semantics is inconsistent or empirically empty |
| self-location, weights and bearer continuity are rival response strategies, not settled facts | `[I/C]` | an interpretation-independent account supersedes them |
| treating parallel blocks as actual is a D5→D4 type promotion | `[S]` | show `KSC-03`/`KSC-19` permit it |
| choice is a D4 event, not D5 content | `[S]` | exhibit a selection that is merely possible |
| the account aims at cross-metaphysical compatibility and empirical vulnerability | `[C]` | a metaphysical inconsistency or empirical null result |
| the real exposure is `GP-02`/`μ₄` | `[C]` | a null result there — which would fire, not fail |

**What this document does not do.** It does not settle quantum interpretation, does
not derive Born weights, does not decide eternalism versus presentism, and does
not close `GP-08`. It shows that the agency account does not depend on any of
them.

•   ⊙   ○ — *the block does not bypass the choosing; the choosing is in the block. What could still undo it is an experiment, not a metaphysic.*
