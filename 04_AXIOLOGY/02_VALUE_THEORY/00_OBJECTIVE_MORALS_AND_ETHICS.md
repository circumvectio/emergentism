...
**Claim Boundary:** `[I/S]` "Objective" here means framework-objective: once reciprocal closure, `P_node = Φ × V`, real boundary relations, and enforced `η = 0` are accepted, better and worse are not matters of taste but directions relative to the invariant. Search handles: objective morality, objective ethics, objective dharma, syntropic morality, syntropic ethics. Objective morality is the `H -> i` vector; objective ethics is the `i -> H` vector; objective dharma is both vectors raising `P_node,i` and `P_node,H` together. Syntropic morality and syntropic ethics mean this dyad is compounding without extraction. This note does not claim to compel agents who reject those premises, and it does not present itself as a completed external proof of moral realism.
