...
| ~~`CITATION-COLLISIONS`~~ | **CLOSED FOR THE DECLARED ACTIVE/CURRENT SCOPE — exact custody landed 2026-08-01; namespace snapshot refreshed 2026-08-09.** All **101 physically reused receipt prefixes** remain unsafe as bare numeric citations; the legacy dangerous-citation heuristic is held at **94**. The active registry binds typed locators and exact target tokens across its fixed owner set and citation-scannable current/provisional public dependencies. Historical/report-only bodies were measured but not renamed or rewritten. | — | `00_META/ACTIVE_RECEIPT_CITATION_REGISTRY.json`; `09_TOOLS/01_SCRIPTS/check_active_receipt_citations.py`; `11_UPLINK/50_AUDITS_AND_EXECUTIONS/243_PUBLIC_RELEASE_PREFLIGHT_AND_CONTACT_SNAPSHOT_2026_08_09.md` |
...
| `OWNER_GATE_HELD_PUBLIC_DOCS` | Which byte-identical numbered-doctrine-spine copy is current planning owner, and what K3 custody does the other retain? **No selection is implied; the preselection guard keeps both copies regular, byte-identical, and excluded by the current local/predeploy deployment configuration.** | public planning/custody routing | `../00_META/00_CONTACT_LIMITED_OWNER_DECISION_DOCKET_2026_08_02.md` (`D-OWNER-01`) |
| `OWNER_GATE_OPEN_TOPOLOGY` | How must the three grandfathered tombstones under `08_FRAMEWORK_SUPPORT/00_META/` be disposed under the categorical root-only rule while preserving custody? **No move or conformance is implied; the preselection guard holds only the exact three hash-bound tombstones, reports the topology debt, and rejects any changed, missing, added, or symlinked entry.** | held topology violation | `../00_META/00_CONTACT_LIMITED_OWNER_DECISION_DOCKET_2026_08_02.md` (`D-OWNER-02`) |
| `FPE-REVIEW-CONTACT-AUTHORITY` | Who may make any later independent-review contact, under which bounded terms and ethics/applicability route? **No reviewer or contact is named.** | six review prerequisites and any invitation | `../00_META/00_CONTACT_LIMITED_OWNER_DECISION_DOCKET_2026_08_02.md` (`D-OWNER-03`) |
| `DOC49-S4-HORIZON-REASSIGNMENT` | Sign or refuse the reassignment of countable-unending traversal from the horizon seat to the realm seat. **STAGED INTO `43` §4/§5/§7 on 2026-09-11; owner disposes; a revert is refusal.** Was pending 37 days in no manifest — this row is the repair of that gap, not of the question. Propagation after signature: `13_BOOKS/titans/RESEARCH_EDITION_1*.md` (staged notes added); `00_HANDOFF/SESSION_AUDIT_2026_08_05.md` (receipt, left as history). | `43 §7` typed table; any public surface teaching "count without end" | `05_COSMOLOGY/01_THE_TRANSCENDENTAL_TRINITY/49_THE_THREE_MODES_OF_COUNTING.md` §4; `00_HANDOFF/2026_09_11_titan_claims_adjudication/README.md` §3b |
| `CLAIM-REGISTER-SCHEMA-TITAN-COMPOUNDS` | Which bucket, if any, admits a `COMPONENT-SUPPORTED` compound with a `last_move` triple? Four Titan-seat compounds are **schema-blocked**: `typed_survivors` is `FORMALLY-VALID`-only and forbids `last_move`; `investigations` is pinned to exactly `RQ-01…09`; `open` takes wager IDs owned by the conjectures ledger; `graves` is constitutional. **No row was opened. The validator was not widened.** `grep -c -i "finity"` on the register therefore stays at 0 until the owner rules on a bucket. Proposed rows: `00_HANDOFF/2026_09_11_titan_claims_adjudication/PROPOSED_REGISTER_ROWS.yaml`. | the gate that would catch the next restatement | `00_META/00_THE_CLAIM_STATUS_REGISTER.md` §3a; `09_TOOLS/01_SCRIPTS/check_claim_status.py` |
...
| **The review packet** — artifact-only professional review can avoid participant research, but contact still has explicit prerequisites | **v6 acyclic hash binding under the v4-profile provenance firewall verified locally 2026-08-23; CONTACT BLOCKED; NOT SENT.** The mechanical manifest binding is real, but the six review prerequisites remain missing. `D-OWNER-03` is unset and the v4-profile accepts no local owner/external evidence or present external state; any future route needs a new reviewed schema and independent verification boundary. Identifying and asking a qualified outsider remains an owner act; *"AI or project-agent review … does not satisfy this external gate."* | `.../REVIEW_BUNDLE_v6.md`; `.../REVIEW_BUNDLE_BINDING_CONTRACT_v2.md`; `.../GATE_REGISTRY.json` |
| **Returned outcomes from outside** | **336 receipt files under the filename convention · 321 citable numbered receipt targets · 0 accepted external-evidence records** | `00_META/CONTACT_LIMITED_STATE.json` · `12_PUBLIC_SITE/record/` |
...
| "86 ambiguous numbers" | The namespace has **three lanes** whose numbered-file counts disagree across sources; the machine source of truth is `11_UPLINK/50_AUDITS_AND_EXECUTIONS/00_RECEIPT_DISAMBIGUATION_INDEX.json`, regenerated by `build_receipt_disambiguation.py` — **101 ambiguous physical prefixes, worst names 4 documents**. The older dangerous-citation heuristic is separately held at **94**. Do not hand-write a lane triple here; it has been wrong three times. **Both live lanes begin at 100 and run over each other for a hundred consecutive integers.** |
