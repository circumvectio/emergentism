---
rosetta:
  primary_level: L2
  primary_column: Philosophy
  operator: "Kālī 💀"
  tier: "God"
  regime: "Śūdra"
  register: "[I]"
  canonical_phrase: "01_EMERGENTISM/02_EPISTEMOLOGY/03_MEMETICS/CLAUDE.md — Agent First-Touch"
---

# 03_MEMETICS — Agent First-Touch

Read [`AGENTS.md`](AGENTS.md) first. This file exists for Claude-style tools that look for `CLAUDE.md`; it narrows the lane and provides read order.

## Read order

1. [`README.md`](README.md) — folder purpose and inventory
2. [`AGENTS.md`](AGENTS.md) — agent routing, caste dispatch, scope boundaries
3. [`../AGENTS.md`](../AGENTS.md) — parent lane authority

## Scope

- 03 Memetics lane of the Emergentism framework.
- How ideas replicate, mutate, and compete — not whether they are true.
- Interpretive exception; not a formal evidence-tier surface.
- Doctrine and framework claims defer to source documents in this folder.
- Evidence tiers mandatory on all claims.

## Exception discipline

| Context | Rule |
|---|---|
| Memetics analysis | May describe spread patterns, selective pressures, and replication fidelity. |
| Truth claims | Must route through [`../01_EVIDENCE_TIERS/00_THE_HONEST_POSITION.md`](../01_EVIDENCE_TIERS/00_THE_HONEST_POSITION.md), not through memetics. |
| Tier movement | Memetics does NOT upgrade a claim's evidence tier. |

## virality≠veracity boundary

- A claim's popularity is independent of its truth value.
- Viral spread may indicate selective advantage, not epistemic validity.
- Do not conflate replication fitness with evidence quality.

## Routing

- Inherit local lane authority from [`AGENTS.md`](AGENTS.md).
- Route parent-lane questions through [`../AGENTS.md`](../AGENTS.md).
- Source truth lives upstream; repair source before patching summaries.

## Cross-cutting laws

- **K2:** every irreversible act requires natural-person signature.
- **η = 0:** zero extraction; fees only on value created.
- **K3:** no-deletion; archive-first.
- **K4:** grace exit; leave with everything.
- **A7:** self-correction with evidence tier on every claim.

## Constraint

Do not treat a clean sevenfold mapping as proof. Rosetta signatures are structural unless independently evidenced.
