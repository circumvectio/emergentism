...
## The rule for everything in `03_MEMETICS/`
1. **Every mechanism claim about the brain/behaviour in these files is `[I]` (interpretive), not `[B]`.** "Memes hijack the CNS," "precisely maps onto viral contagion," "parasitic replicators" are *models and analogies*, not settled neuroscience. Read every such phrase as if prefixed "*on this model* …".
2. **Separate the tiers that the prose currently blends:**
   - **`[B]` — the custodied history.** Documented historical cases (Shaker collapse, Cathar episodes, chain-letter dynamics) are observation/attribution claims whose sources must remain named.
   - **`[A]` — the mathematics.** The SIR epidemic equations qualify only as formal mathematics inside their declared assumptions.
...
- Kill criterion for this sub-lane: if a memetics mechanism claim is ever cited as `[B]`/settled empirical science anywhere downstream, it has drifted — route back here and re-tier.
