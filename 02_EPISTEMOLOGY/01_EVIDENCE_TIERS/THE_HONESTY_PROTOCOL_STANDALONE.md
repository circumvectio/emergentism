...
| **Irreversibility gate** | Auto-escalation as a *reversible pilot* on a small randomized slice: possible on `[C]` with scoped authorization. Auto-escalation hard-wired as the default for every account creates a wider consequence surface — the gate says no until mandate, consent, contest, actor, and consequence bearers are explicit. |
