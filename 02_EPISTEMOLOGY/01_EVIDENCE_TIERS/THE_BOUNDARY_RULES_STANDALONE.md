...
**This manual uses it for the interior region**, on the interval-notation precedent: `[0, 1]` mixes sorts — bracket, element, comma, element, bracket — and is universally readable, because the *arrangement* says which is which. Boundary, interior, boundary is that same shape. What matters is not that the three marks share a sort but that their **roles are fixed by position** and declared once. This is also why nothing may be written in the space between them (rule N above): an infix symbol both promises an operation that does not exist and overwrites the only depiction of the interior you have.

*Draft-1 history, left visible because it is the kind of churn a reader should be able to see: this began as "region", was changed to "point" to match a source-project ruling of 2026-08-05, and was changed back the same day when that ruling was reversed by the project owner. **It is back to "region."** The manual's own advice survived all three states unchanged — declare the sort, keep the three marks readable, and do not write in the space between them. That the advice was stable while the choice was not is the reason the advice is the part worth keeping.*
