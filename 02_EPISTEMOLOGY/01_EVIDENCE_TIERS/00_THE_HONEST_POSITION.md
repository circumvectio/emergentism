...
| ~~A6~~ **S15** | Multiplicative fitness structures generate cooperation in AI agents; additive structures do not | Protocol D, this framework | Confirmed within framework-designed simulations: d = 6.66, p = 2.02 × 10⁻¹². **Upgrade criterion:** if an independent research team (no authorship overlap with EFR), using a pre-registered protocol not designed within EFR, confirms multiplicative cooperation with effect size d > 2.0 and p < 0.01, S15 upgrades to [A]. |
| ~~A7~~ **S16** | In simulated agents, V×Φ perfectly predicts performance ranking | Agent V×Φ simulation | Confirmed within framework-designed simulations: Spearman ρ = 1.000. **Upgrade criterion:** if an independent team replicates V×Φ predictive dominance over additive models with ρ > 0.8 in a pre-registered protocol, S16 upgrades to [A]. |
...
**What these results mean:** The mathematics of ℂP¹ is real (A1-A5). The multiplicative structure produces measurably different outcomes than the additive structure in framework-designed agent simulations (S15-S16). A1-A5 are facts independent of the framework. S15-S16 are confirmed within the framework's own testing environment and require independent replication to earn [A]/[B] status.
...
5. **That negative results will be hidden.** Protocol D is confirmed within framework-designed simulations. SPHERE has a first negative public-model run and remains open. GFS now has mixed reported results. If stronger evidence shows the additive model wins cleanly, the framework publishes the failure. This commitment is non-negotiable. A framework that hides negative results is not approaching truth — it is approaching ideology.
...
| **Protocol D** | Multiplicative cooperation in AI agents | **CONFIRMED** (d=6.66) | Done |
