...
| OS01-09 | §6. **Retain → restate.** | K-1, K-3 E6, K-4 W3–W4 / typed profile `[S]`; calibrated application `[I/C]`. | Ordinal factors license componentwise Pareto comparison, not a scalar. Product, minimum, additive, CES, a single feasible-cone score, or no aggregation may perform better after calibration. Ranking reversal under an admissible transformation kills the scalar use; the map/means distinction survives. | “Keep possible-power evaluation and actual means as a two-axis profile; any scalar requires an explicit calibration and rival test.” |
