...
The site has been trying to present *an account*. It should present **a map** —
specifically:
...
obstruction is nonzero Gaussian curvature, not curvature in the everyday
sense."* The theorem does not say *curved things cannot be flattened*. It says
**exactly which regions must tear, and which are flat all along.** That is a
legend, not a lament.
...
Six top-level nodes, matching the `D-20` WorkFlowy grammar already ruled for
the estate: branch ring and leaf dot, search, breadcrumb focus, disclosure,
summaries, current-route state.

```text
•  ⊙  ○
├─ ○ The Gestalt              the whole, in one screen
├─ ○ The Ladder               D0 → D6 — each register expands to:
│                               what is the case · how we read it
│                               who else arrived here · what the crossing costs
├─ ○ The Insights             ours, named and tiered
├─ ○ The Convergence          independent arrivals — the asset
├─ ○ The Frontier             open questions + Extend the map →
└─ · Exit                     leaf, top-level, never behind a disclosure
```

**The three scripts are disclosure states, not siblings.** Depth is the
disclosure axis in a WorkFlowy grammar, so:

| state | script | shows |
|---|---|---|
| collapsed | `/fable/` | the node's one line |
| expanded | `/plainly/` | what it says and why it matters |
| fully expanded | `/record/` | tier, owner, rival, discriminator, kill |

The transmission strategy becomes an affordance rather than a menu, and
`breadcrumb focus` lets any node become the root — which is what 81 flat routes
need.

## 4 · Where the workshop goes

**The audit is a production discipline, not a presentation strategy.** It is
*why* the claims are good; it is not the front door. A manufacturer publishes
...
Each node carries our reading, its independent arrivals, and its fences. **AI
then proposes the next neighbour**: a discovery adjacent to this node that
nothing in the map yet links.

- The map is never finished — and that is the doctrine, not a defect.
- Contribution costs one link, not a manuscript.
- **The frontier is the front page.** Gaps are invitations, not admissions.
- The corpus becomes a **reference rather than a position.** People return to a
  map; they argue with a position once and leave.

It also dissolves the priority problem by inverting it: stop competing for a
fixed prize, hold the map of who-found-what — which nobody is competing for,
and which only became buildable when reading capacity stopped being bounded by
one lifetime.

## 7 · Two lines that must never be crossed

**Never "first."** A bounded map whose limits are drawn is Kant's critical
project and the Tractatus and the whole via negativa. Six candidates were
pre-empted in one day this month; this idea would be pre-empted too. What is
plausibly distinctive is the **instrument**: the bound drawn by a *census of
proven theorems, each tiered*, rather than by one master argument. Kant drew
the line with a transcendental deduction; this draws it with 127 impossibility
results and a legend. That is a method claim, and it is the only one worth
making because it is the only one that survives an audit.

**Never "more accurate."** No-free-lunch: a projection is never more accurate
*simpliciter*. Mercator is more accurate *for navigation* and worse for area.
The defensible sentence is **"this map declares its purpose and its
distortions, where others hide theirs."** Cross that line and a cartographer
kills the claim in one sentence.

## 8 · Implementation notes for whoever builds it

- **Both geometry pages are GENERATED** — `burrisphere` by `build_library_index.py`
  and `build_pwa.py`; `riemann` by `build_library_index.py` and
  `render_dimension_site.py`. Hand-patching will be overwritten; repair at the
  generator. `generate_public_library.py`'s own docstring documents this failure
  mode explicitly.
- **38 pages already ship `<details>` disclosure** and `atlas-drawer.js` exists.
  Check for collision before introducing a second disclosure system.
