...
rosetta:
  ...
  regime: "Kṣatriya"
  register: "[S]"
  canonical_phrase: "K2 envelope — migrate current 12_PUBLIC_SITE/book-pwa source to 02_SKYZAI/03_AIA/app"
...
status: "SIGNED 2026-05-31 (Yves R. Burri) — K2 GATE CLEARED. The mortal performed the binding act of authorization. Physical execution coordinates with the AIA engine lane (spec 13, mid-consolidation); book-pwa lands once 02_SKYZAI/03_AIA/app/ is ready, not into a half-built tree."
...
> coordinates. Physical migration to `02_SKYZAI/03_AIA/app/` remains blocked until the
> AIA lane has a ready destination and a final signoff receipt.

## The act (one line)

Current executable interpretation: move the frozen app source **`01_EMERGENTISM_ORG/12_PUBLIC_SITE/book-pwa/`** → **`03_AIA/app/`** only after the AIA destination and final signoff receipt exist. The signed 2026-05-31 text used `10_PUBLIC_SITE/book-pwa`; that string is preserved below as historical signature provenance, not as a shell path.

## Why (the boundary the move honours)

The prime directive of `02_SKYZAI/02_AIA` is *"the weltanschauung is content, not code."* The `book-pwa` is the **worldview-agnostic engine** (the AIA medium) — it is **AIA, not discovery**, and is currently buried inside the discovery root. The canonical target layout is the parallel-AI architecture spec **[`02_SKYZAI/03_AIA/13_APP_BOOKS_SEPARATION_2026_05_31.md`](../../02_SKYZAI/03_AIA/13_APP_BOOKS_SEPARATION_2026_05_31.md)**: engine in `02_SKYZAI/03_AIA/app/`, content in `02_SKYZAI/03_AIA/EMERGENTISM_AIA/`, bridged only by `worldview.manifest.json`. This envelope is the **source-side K2 surface**; the AIA lane owns the engine and **verifies after** (per spec 13: *"whoever reorganizes executes from this; I verify after"*).
...
2. **Concurrent engine work.** The AIA lane is actively consolidating `app_scaffold/` + `workflowy-clone/` + the buried `INFINITE_BOOK_PRODUCT` into `02_SKYZAI/03_AIA/app/` (spec 13). The move must **land into their target, not collide with it** — coordinate timing; do not move into a half-built `02_SKYZAI/03_AIA/app/`.
...
1. **ENVELOPE-1 — the move.** `git mv 01_EMERGENTISM/12_PUBLIC_SITE/book-pwa → 02_SKYZAI/03_AIA/app` (or merge into the AIA-lane's staged `02_SKYZAI/03_AIA/app/` per their direction). Existence-guarded relink of inbound references. `npm install && npm run build` at the destination — **must pass.**
2. **ENVELOPE-2 — K3 tombstone.** Leave `12_PUBLIC_SITE/00_BOOK_PWA_MOVED.md` redirecting to `03_AIA/app/` (archive-first; never silent-delete). Update `CLAUDE.md` and public-site route cards to read "moved → `03_AIA/app/`".
...
- [ ] `npm run build` green at `02_SKYZAI/03_AIA/app/`
...
```
ACT: Move 10_PUBLIC_SITE/book-pwa → 02_SKYZAI/03_AIA/app (engine/content separation, spec 13)
...
Notes / conditions:  Authorized. Physical execution coordinates with the AIA
  lane: 02_SKYZAI/03_AIA/app/ is an empty shell mid-consolidation (app_scaffold +
