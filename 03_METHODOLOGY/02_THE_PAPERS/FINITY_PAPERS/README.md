...
rosetta:
  ...
  secondary:
    - level: L5
      column: Cosmology
      role: "Finity geometry as formal bridge (Brahmā secondary)"
  register: "[A/S/I]"
...
A rigorous programme on the geometry of `{0, 1, ∞}` — extending Minoru Suda's 2025 reciprocal-symmetry work (cited as a *later independent formalisation*; idea-priority is the author's **2024** statement). Nothing here claims to unify, fix, prove, or solve; each paper claims one structure at its honest tier, and carries an explicit way to die.

| # | Paper | Home | Thesis | Tier |
|---|---|---|---|---|
...
| **III** | A Product Constraint as an Ethical Case | `PAPER_III_A_PRODUCT_CONSTRAINT_AS_A_STRUCTURAL_ETHIC.md` | reciprocal arithmetic supplies a descriptive constraint only; Justice is the declared ethical floor, and no ought follows from the product | [A] algebra · [S/I] selected ethic |
...
| **VI** | Emergence as the Lens on Dasein | `PAPER_VI_EMERGENCE_AS_LENS_ON_DASEIN.md` | **superseded draft**: retained for genealogy; current μ claims route through doc 48 and KSC-05, not dimensional-crossing shorthand | historical `[D]` |
...
**The one discipline they share:** every claim tier-marked `[A]/[B]/[S]/[I]/[C]`, never upgraded; Titan seats remain operator-free, while any reciprocal product is stated only for finite nonzero chart coordinates; the geometry is a lens, never the Ground. The papers approach the limit by being *refutable* — the only kind worth publishing.
