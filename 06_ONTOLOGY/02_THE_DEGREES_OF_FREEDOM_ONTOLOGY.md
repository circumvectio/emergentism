...
**3.2 Where the strong claim fails — the honest cut.** The filter selects the **basis** (which observables become definite), **not the outcome** (which definite value). *Many* branches are equally coherent and consistent — every detector position in the double slit is a perfectly consistent history — so **"the only one that is coherent" is false**; the puzzle is *which* random one, with *which* weights. Decoherence explains the emergence of classicality; it does **not**, alone, deliver definite outcomes.

**3.3 Interpretation remains open.** Everettian branch actuality and indexical self-location are one possible interpretation, not a consequence forced by the plenitude wager. Collapse, Bohmian, relational, objective-collapse, and other approaches type actuality differently. The interpretation-neutral survivor is narrower: interaction, amplification, environment, and decoherence can help explain stable macro-record formation. Definite-outcome selection—if required by the chosen ontology—and Born weights remain separate debts.
